...
int setLocale(char* argv1)
{
    ...
    LOG(LogInfo) << "Locale set to ... " << std::use_facet<boost::locale::info>(std::locale()).name() ;
    return 0;
}
...
//    boost::filesystem::path::imbue(std::locale());
    ...
    //start the logger
    Log::open();
    LOG(LogInfo) << "CK - EmulationStation - v" << PROGRAM_VERSION_STRING << ", built " << PROGRAM_BUILT_STRING;
